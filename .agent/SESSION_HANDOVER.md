...
網站完整上線,頁面順序:hero 像素世界 → 證物板 10 作品(連結全數填妥)→「文」文章區(iThome 鐵人賽兩系列+嘎姆期刊受訪)→「磚」MC 地圖區
...
有 video 就不顯示靜態截圖),目前小女巫有影片。footer 有信箱 b43096022@gmail.com。
favicon 是像素風金色圖釘,由 16x16 像素格產出 svg + ico + favicon-180.png,
改造型改 `scripts/gen-favicon.cjs` 裡的像素格再 `node scripts/gen-favicon.cjs`
