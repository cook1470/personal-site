...
- HTML `hidden` 屬性會被自己 CSS 的 `display: flex/grid/inline-block` 蓋掉（UA 樣式必輸給
...
- CSS 高度容易被壓掉的兩個地方（改版面時反覆踩到）：
  圖片的內在高度在 flex/grid 子項裡不可靠，要固定比例就用 `padding-top: 56.25%` 的盒子；
  grid 的 `1fr` 最小值是 `auto`，格子會被內容撐開而溢出，要寫 `minmax(0, 1fr)`。
  另外容器給了固定高度、內部又要撐開內容時，子項需要 `min-height: 0` 才縮得下去。
